...
driver.wait(webdriver.until.titleMatches(/: Complete$/), 10000).then(() => {
    driver.executeScript("return document.documentElement.outerHTML").then((html) => {
        fs.writeFile("test-result.html", html, (err) => {
            if (err) throw err;
            const tests = driver.findElements(webdriver.By.cssSelector(".test"));
            const passedTests = driver.findElements(webdriver.By.cssSelector(".test.pass:not(.pending)"));
            const failedTests = driver.findElements(webdriver.By.cssSelector(".test.fail"));
            const pendingTests = driver.findElements(webdriver.By.cssSelector(".test.pending)"));
