    ...
    driver.executeScript(`return document.querySelectorAll("#mocha-report")`).then((rt) => {
        driver.executeScript("return document.documentElement.outerHTML").then((html) => {
            fs.writeFile("test-result.html", html, (err) => {
                if (err) throw err;
                const tests = rt.querySelectorAll(".test");
                const passedTests = rt.querySelectorAll(".test.pass:not(.pending)");
                const failedTests = rt.querySelectorAll(".test.fail");
                const pendingTests = rt.querySelectorAll(".test.pending)");
