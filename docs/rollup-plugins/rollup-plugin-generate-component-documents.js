...
function exists(filePath) {
  return new Promise((resolve, reject) => fs.access(filePath, resolver(resolve, reject)));
}

async function outroContent({ dest, examplePath }) {
  try {
    await exists(path.resolve(dest, '..', examplePath));
    return `
## Example

{% include example.html url='/${examplePath}' %}
...
function getExamplePath(src, ext) {
  return `examples/${getRelativePath(src, ext)}.html`;
}
