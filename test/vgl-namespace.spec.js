describe('VglNamespace component', function component() {
  const { VglNamespace } = VueGL;
  const { assert } = chai;
  describe('Global datas (cameras and scenes)', function suite() {
    it('Descendants should be able to inject vglCameras and vglScenes.', function test(done) {
      const vm = new Vue({
    ...
    it('Provided datas should be global under the root VglNamespace component.', function test(done) {
      const vm = new Vue({
  ...
  describe('Local datas (geometries and materials)', function suite() {
    it('Descendants should be able to inject vglGeometries and vglMaterials.', function test(done) {
      const vm = new Vue({
        template: '<vgl-namespace><child-component ref="child" /></vgl-namespace>',
        components: {
          VglNamespace,
    ...
    it('Each VglNamespace component should provide datas locally.', function test(done) {
      const vm = new Vue({
    ...
    it('The datas should be reactive.', function test(done) {
      ...
      const vm = new Vue({
        ...
        components: {
          ...
          ChildComponent: {
            ...
            inject: ['vglGeometries', 'vglMaterials'],
            watch: {
              'vglGeometries.forGet': function forGet() {
                geometryChanges += 1;
              },
              'vglMaterials.forGet': function forGet() {
                materialChanges += 1;
              },
            },
          },
    ...
    it("The ancestor's datas should also be reactive.", function test(done) {
      let geometryChanges = 0;
      let materialChanges = 0;
      const vm = new Vue({
        template: '<vgl-namespace><parent-component ref="parent" /><vgl-namespace><child-component ref="child" /></vgl-namespace></vgl-namespace>',
