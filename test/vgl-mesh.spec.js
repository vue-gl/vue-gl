describe('VglMesh:', function suite() {
  const { VglMesh, VglObject3d, VglNamespace } = VueGL;
  const { expect } = chai;
  let updatedHistory;
  const ObjectWatcher = {
  ...
  it('default', function test(done) {
    const vm = new Vue({
      template: '<object-watcher><vgl-mesh /></object-watcher>',
      components: { VglMesh, ObjectWatcher },
    }).$mount();
    after10ticks(vm, () => {
      try {
        ...
        expect(actual).to.have.property('type', 'Mesh');
        expect(actual.position.equals(new THREE.Mesh().position)).to.equal(true);
        expect(actual.quaternion.equals(new THREE.Mesh().quaternion)).to.equal(true);
        expect(actual.scale.equals(new THREE.Mesh().scale)).to.equal(true);
        done();
      } catch (e) {
        done(e);
      }
  ...
  it('with position, rotation, and scale property', function test(done) {
    const vm = new Vue({
      template: '<object-watcher><vgl-mesh position="-0.5 0.5 2" rotation="3 1 1 YZX" scale="1.2 1.5 0.8" /></object-watcher>',
      components: { VglMesh, ObjectWatcher },
    }).$mount();
    after10ticks(vm, () => {
      try {
  ...
  it('after position property is changed', function test(done) {
    const vm = new Vue({
      template: '<object-watcher><vgl-mesh :position="position" /></object-watcher>',
      components: { VglMesh, ObjectWatcher },
    ...
    vm.$nextTick(() => {
      ...
      after10ticks(vm, () => {
        try {
          ...
          expect(actual1).to.have.property('type', 'Mesh');
          expect(actual1.position.equals(new THREE.Vector3(0.5, 1, -0.5))).to.equal(true);
          expect(actual1.quaternion.equals(new THREE.Mesh().quaternion)).to.equal(true);
          expect(actual1.scale.equals(new THREE.Mesh().scale)).to.equal(true);
          expect(updatedHistory[1].children).to.have.lengthOf(1);
          const [actual2] = updatedHistory[1].children;
          expect(actual2).to.have.property('type', 'Mesh');
          expect(actual2.position.equals(new THREE.Vector3(0, 0, 0.5))).to.equal(true);
          expect(actual2.quaternion.equals(new THREE.Mesh().quaternion)).to.equal(true);
          expect(actual2.scale.equals(new THREE.Mesh().scale)).to.equal(true);
          done();
        } catch (e) {
          done(e);
        }
  ...
  it('after rotation property is changed', function test(done) {
    const vm = new Vue({
      template: '<object-watcher><vgl-mesh :rotation="rotation" /></object-watcher>',
      components: { VglMesh, ObjectWatcher },
    ...
    vm.$nextTick(() => {
      ...
      after10ticks(vm, () => {
        try {
          ...
          const [actual1] = updatedHistory[0].children;
          expect(actual1).to.have.property('type', 'Mesh');
          expect(actual1.position.equals(new THREE.Mesh().position)).to.equal(true);
          expect(actual1.quaternion.equals(new THREE.Quaternion().setFromEuler(new THREE.Euler(0.5, 1, -0.5, 'XYZ')))).to.equal(true);
          expect(actual1.scale.equals(new THREE.Mesh().scale)).to.equal(true);
          expect(updatedHistory[1].children).to.have.lengthOf(1);
          const [actual2] = updatedHistory[1].children;
          expect(actual2).to.have.property('type', 'Mesh');
          expect(actual2.position.equals(new THREE.Mesh().position)).to.equal(true);
  ...
  it('after scale property is changed', function test(done) {
    const vm = new Vue({
      template: '<object-watcher><vgl-mesh :scale="scale" /></object-watcher>',
      components: { VglMesh, ObjectWatcher },
