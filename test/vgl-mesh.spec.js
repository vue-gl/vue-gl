describe('VglMesh:', function suite() {
  ...
  it('without properties', function test(done) {
    const vm = new Vue({
      template: '<vgl-namespace><vgl-material name="m" ref="m" /><vgl-geometry name="g" ref="g" /><vgl-mesh material="m" geometry="g" ref="o" /></vgl-namespace>',
      components: {
        VglNamespace,
        VglMaterial,
        VglGeometry,
    ...
    vm.$nextTick(() => {
      try {
        ...
        const expected = new THREE.Mesh(new THREE.BufferGeometry(), new THREE.Material());
  ...
  it('with properties', function test(done) {
    const vm = new Vue({
      template: '<vgl-namespace><vgl-material name="m" ref="m" /><vgl-geometry name="g" ref="g" /><vgl-mesh material="m" geometry="g" position="8 3 -3.5" rotation="0.8 0.8 0.5 XZY" scale="1.3 1.4 1.1" ref="o" /></vgl-namespace>',
      components: {
        VglNamespace,
        VglMaterial,
        VglGeometry,
  ...
  it('after properties are changed', function test(done) {
    const vm = new Vue({
      template: '<vgl-namespace><vgl-material name="m" ref="m" /><vgl-material name="ma" ref="ma" /><vgl-geometry name="g" ref="g" /><vgl-geometry name="ga" ref="ga" /><vgl-mesh :material="m" :geometry="g" :position="p" :rotation="r" :scale="s" ref="o" /></vgl-namespace>',
      components: {
        VglNamespace,
        VglMaterial,
        VglGeometry,
