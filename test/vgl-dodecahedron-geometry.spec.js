describe('VglDodecahedronGeometry:', function suite() {
  ...
  it('without properties', function test() {
    const vm = new Vue({
      template: '<vgl-namespace><vgl-dodecahedron-geometry name="abc1#2" /><geometry-watcher geometry="abc1#2" /></vgl-namespace>',
  ...
  it('with properties', function test() {
    const vm = new Vue({
      template: '<vgl-namespace><vgl-dodecahedron-geometry name="abc1#2" radius="22.24" detail="3" /><geometry-watcher geometry="abc1#2" /></vgl-namespace>',
      components: { VglNamespace, VglDodecahedronGeometry, GeometryWatcher },
    }).$mount();
    return vm.$nextTick().then(() => {
      expect(updatedHistory).to.have.lengthOf(1);
      const actual = updatedHistory[0].vertices;
      const expected = new THREE.DodecahedronGeometry(22.24, 3).vertices;
      expect(actual).to.have.deep.ordered.members(expected);
    });
  });
  it('after radius property is changed', function test() {
    const vm = new Vue({
      template: '<vgl-namespace><vgl-dodecahedron-geometry name="abc1#2" :radius="radius" /><geometry-watcher geometry="abc1#2" /></vgl-namespace>',
      components: { VglNamespace, VglDodecahedronGeometry, GeometryWatcher },
      data: { radius: 26 },
    }).$mount();
    return vm.$nextTick().then(() => {
