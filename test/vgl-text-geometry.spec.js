...
import { VglTextGeometry, VglNamespace } from '../src';
...
describe('VglTextGeometry:', () => {
  ...
  test('with properties', (done) => {
    ...
    new FontLoader().load(font1, (font) => {
      const expected = new TextBufferGeometry('a', {
        font,
        size: 120,
        height: 6,
        curveSegments: 8,
        ...
        if (inst.type === expected.type) {
          vm.$nextTick(() => {
            try {
              ...
              expect(mediator.copy(inst).toJSON()).toEqual(mediator.copy(expected).toJSON());
              done();
            } catch (e) {
              done(e);
            }
  ...
  test('without properties', (done) => {
    ...
    new FontLoader().load(font1, (font) => {
      const expected = new TextBufferGeometry('a', { font });
      vm.$refs.g.$watch('inst', (inst) => {
        if (inst.type === expected.type) {
          vm.$nextTick(() => {
            try {
      ...
      const unwatch1 = vm.$refs.g.$watch('inst', (inst1) => {
        if (inst1.type === expected1.type) {
          vm.$nextTick(() => {
            ...
            vm.$nextTick(() => {
              try {
                const expected2 = new TextBufferGeometry('a', {
                  font: fontBefore,
                  size: 152.3,
                  height: 7,
                  curveSegments: 6,
              ...
              vm.$refs.g.$watch('inst', (inst2) => {
                new FontLoader().load(font2, (fontAfter) => {
                  try {
                    const expected3 = new TextBufferGeometry('a', {
                      font: fontAfter,
                      size: 152.3,
                      height: 7,
                      curveSegments: 6,
