...
import { CameraHelper, PerspectiveCamera, LineSegments } from 'three';
import { VglCameraHelper, VglPerspectiveCamera, VglNamespace } from '../src';

describe('VglCameraHelper component', () => {
  test('creating a helper', (done) => {
    const vm = new Vue({
    ...
    vm.$nextTick(() => {
      try {
        ...
        const camera = new PerspectiveCamera(52, undefined, 0.11, 2100);
    ...
    vm.$nextTick(() => {
      ...
      vm.$nextTick(() => {
        try {
          ...
          const uuids = { geometry: mediator.geometry.uuid, material: mediator.material.uuid };
          const actual = mediator.toJSON();
          const camera = new PerspectiveCamera(54, undefined, 0.12, 1200);
          const helper = new CameraHelper(camera);
          mediator.copy(helper);
          mediator.geometry = helper.geometry;
          mediator.material = helper.material;
