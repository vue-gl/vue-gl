describe("VglFont component", function() {
    ...
    describe("The instance should be registered to the injected namespace.", function() {
        it("Null should be registered when created.", function() {
            const vm = new Vue({
                template: `<vgl-namespace><vgl-font name="dm'&^>" src="base/test/helvetiker_regular.typeface.json" /><other-component ref="other" /></vgl-namespace>`,
                components: {
                    VglFont,
                    VglNamespace,
                    OtherComponent: {
        ...
        it("Should be replaced when the typeface is loaded.", function(done) {
            const vm = new Vue({
                template: `<vgl-namespace><vgl-font name="'<!--" src="base/test/helvetiker_regular.typeface.json" ref="f" /><other-component ref="other" /></vgl-namespace>`,
                components: {
                    VglNamespace,
                    VglFont,
                    OtherComponent: {
        ...
        it("Should be unregistered when destroyed.", function(done) {
            const vm = new Vue({
                template: `<vgl-namespace><vgl-font name="n<!--" src="base/test/helvetiker_regular.typeface.json" v-if="!destroyed" ref="f" /><other-component ref="other" /></vgl-namespace>`,
                components: {
                    VglFont,
                    VglNamespace,
                    OtherComponent: {
        ...
        it("Should be replaced when the instance is replaced.", function(done) {
            const vm = new Vue({
                ...
                data: {src: "base/test/helvetiker_regular.typeface.json"}
            }).$mount();
            const unwatch = vm.$refs.f.$watch("inst", (before) => {
                unwatch();
                vm.src = "base/test/helvetiker_bold.typeface.json";
