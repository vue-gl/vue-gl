describe("VglTetrahedronGeometryコンポーネントのテスト", function() {
    const {VglTetrahedronGeometry} = VueGL;
    const assert = chai.assert;
    describe("プロパティの確認", function() {
        it("instプロパティはTetrahedronGeometryオブジェクト", function() {
            const vm = new Vue(VglTetrahedronGeometry);
            assert.equal(vm.inst.type, "TetrahedronGeometry");
        });
    });
    describe("プロパティのテスト", function() {
        describe("radiusプロパティ", function() {
            ...
            it("\"20\" -> 20", function() {
                const vm = new (Vue.extend(VglTetrahedronGeometry))({
                    propsData: {radius: "20"}
                });
